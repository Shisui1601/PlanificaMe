...
class MailService:
    ...
    def send_calendar_invite_email(to_email: str, to_name: str, inviter_name: str,
        ...
        {_btn("Abrir PlanificaMe →", settings.FRONTEND_URL, "#7c5aff")}"""
    ...
    def send_event_created_email(
        ...
        {_btn("Ver actividad →", activity_url, type_color)}'''
    ...
    def send_event_updated_email(
        ...
        {_btn("Ver actividad →", activity_url, "#3b82f6")}'''
    ...
    def send_custom_reminder(
        ...
        logger.info(
            "Recordatorio enviado por %s: %d exitoso(s), %d fallido(s)" %
            (sender_name, len(sent_to), len(failed))
        )
